...
from consts import *
from tgcalc import app, engine, cache
...
# Чтение инфо пользователя из кеша
# <token> -- цифровая подпись Tg-бота
# <uid>   -- id Tg-пользователя
# <val>   -- сериализованные данные. необязательный параметр. если указан, то запись в кеш, иначе чтение
@app.route('/user/cache', methods=['GET'])
def user_read_cache():
    token = request.args.get('token', type=str)
    uid   = request.args.get('uid',   type=int)
    ...
    if token == TG_TOKEN:
        if uid:
            ...
            if val:
                ...
                try:
                    val = cache.get(key)
                    val = val.decode() if val else ''
                except:
                    return error_resp(ERR_DB)

            return error_resp(ERR_OK, val)
        ...
        if uid and now:
            try:
                ...
                try:
                    ...
                    for r in res:
                        resp.append({'id': r['id'],                     # id мероприятия
        ...
        if tid is not None and tr is not None:
            try:
                ...
                try:
                    ...
                    return error_resp(ERR_OK, t) if t is not None else error_resp(ERR_DB)
